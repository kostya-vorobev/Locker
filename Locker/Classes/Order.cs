...
namespace CleanPCClub
{
    class Order : Table
    {
        ...
        public static string SelectFinishTime(PC pc)
        {
            pc.SelectPCID();
            MySqlLib.MySqlData.MySqlExecuteData.MyResultData result = new MySqlLib.MySqlData.MySqlExecuteData.MyResultData();
            string selectQuery = "SELECT `order`.Time_Finish FROM `pcclub`.`order` WHERE `order`.Id_PC = "+pc.Id+" ORDER BY `order`.id DESC ";
            result = MySqlLib.MySqlData.MySqlExecuteData.SqlReturnDataset(selectQuery);
            if (result.HasError == false)
            {
                return result.ResultData.Rows[0][0].ToString();
            }
            return "01:00:00";
        }
